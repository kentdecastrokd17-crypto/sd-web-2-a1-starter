...
const brokenCharacters = [
  { id: 11, age: 30 },
  { id: 12, name: "Mace Windu" },
  { id: 13, age: 35 },
  { id: 14, name: "Qui-Gon Jinn", age: 50 },
];
...
function renderCharacterNames(charactersArray, targetListId) {
...
function filterCharactersByAge(charactersArray, ageThreshold, targetListId) {
  const targetList = document.getElementById(targetListId);
  charactersArray.filter(char => char.age < ageThreshold).forEach(char => {
    const li = document.createElement("li");
    li.textContent = `${char.name} (Age: ${char.age})`;
    targetList.appendChild(li);
  });
...
filterCharactersByAge(characters, 40, "age-filter-list");
...
function renderWithValidation(dataArray, targetListId, errorDivId) {
  const targetList = document.getElementById(targetListId);
  const errorDiv = document.getElementById(errorDivId);

  dataArray.forEach(char => {
    if (!char.hasOwnProperty('name')) {
      const errorMsg = 'Error: Character ID ${char.id} is missing the "name" property.';
...
renderWithValidation(brokenCharacters, "broken-characters-list", "broken-array-errors");